...
func (r *ProjectRepository) GetSummary(projectID uint, sampleRepo *SampleRepository, taskRepo *TaskRepository) (*model.ProjectSummaryResponse, error) {
	...
	summary := &model.ProjectSummaryResponse{
		...
		TotalSamples:      int(sampleCounts[model.SampleStatusPending] + sampleCounts[model.SampleStatusProcessing] + sampleCounts[model.SampleStatusCompleted] + sampleCounts[model.SampleStatusFailed]),
		PendingSamples:    int(sampleCounts[model.SampleStatusPending]),
		ProcessingSamples: int(sampleCounts[model.SampleStatusProcessing]),
		CompletedSamples:  int(sampleCounts[model.SampleStatusCompleted]),
		FailedSamples:     int(sampleCounts[model.SampleStatusFailed]),
		TotalTasks:        int(taskCounts[model.TaskStatusQueued] + taskCounts[model.TaskStatusRunning] + taskCounts[model.TaskStatusCompleted] + taskCounts[model.TaskStatusFailed] + taskCounts[model.TaskStatusCancelled]),
		RunningTasks:      int(taskCounts[model.TaskStatusRunning]),
		CompletedTasks:    int(taskCounts[model.TaskStatusCompleted]),
		FailedTasks:       int(taskCounts[model.TaskStatusFailed] + taskCounts[model.TaskStatusCancelled]),
	}
